...
export default function DashboardPage() {
      ...
      try {
        ...
        if (anakRes.ok) {
          ...
          const processedChildren = await Promise.all(
            rawAnakList.map(async (child: any) => {
              try {
                const ukurRes = await fetch(
                  `/api/pengukuran?anakId=${child.id}`
                );
                ...
                if (ukurRes.ok) {
                  ...
                  if (
                    ukurData.pengukuranList &&
                    ukurData.pengukuranList.length > 0
                  ) {
          ...
          const newStats = {
            ...
            normal: processedChildren.filter(
              (c: any) =>
                c.statusGizi.includes("Normal") || c.statusGizi.includes("Baik")
            ).length,

            stunting: processedChildren.filter(
              (c: any) =>
                c.statusGizi.includes("Pendek") ||
                c.statusGizi.includes("Stunted")
            ).length,

            wasting: processedChildren.filter(
              (c: any) =>
                c.statusGizi.includes("Kurus") ||
                c.statusGizi.includes("Wasted") ||
                c.statusGizi.includes("Buruk")
            ).length,

            underweight: processedChildren.filter(
              (c: any) =>
                c.statusGizi.includes("Kurang") &&
                !c.statusGizi.includes("Kurus") // Bedakan kurang BB vs kurus
            ).length,

            belumDiukur: processedChildren.filter(
              (c: any) => c.statusGizi === "Belum Diukur"
            ).length,
          };
  ...
  const filteredChildren = children.filter((child) =>
    child.namaAnak.toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        <div className="mb-8 flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500">
              Pantau perkembangan status gizi anak secara real-time
            </p>
          </div>
          {user?.role === "BIDAN" && (
            <Link
              href="/dashboard/kader"
              className="bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 px-5 py-2.5 rounded-lg font-medium shadow-sm flex items-center transition-all"
            >
              <svg
          ...
          >
            <span className="mr-2 text-xl leading-none">+</span> Tambah Data
            Anak
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <p className="text-xs text-gray-400 mt-4">
              {stats.belumDiukur} anak belum diukur
            </p>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 relative overflow-hidden">
            ...
            <div className="w-full bg-gray-100 rounded-full h-1.5 mt-4">
              <div
                ...
                style={{
                  width: `${
                    stats.total ? (stats.normal / stats.total) * 100 : 0
                  }%`,
                }}
              ></div>
            </div>
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-3xl font-bold text-yellow-600">
                  {stats.wasting + stats.underweight}
                </h3>
              </div>
          ...
          <div className="px-6 py-5 border-b border-gray-200 bg-gray-50/50 flex justify-between items-center">
            <div>
              <h3 className="text-lg font-bold text-gray-900">
                Daftar Anak & Status Gizi
              </h3>
              <p className="text-sm text-gray-500">
                Data terbaru berdasarkan pengukuran terakhir
              </p>
            </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th
                    scope="col"
                    className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider hidden sm:table-cell"
                  >
                    Tgl. Ukur
                  </th>
                ...
                {filteredChildren.length === 0 ? (
                    ...
                    <td colSpan={4} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center justify-center text-gray-500">
                        ...
                        <p className="text-sm mt-1">
                          Silakan tambah data anak untuk memulai pemantauan.
                        </p>
                        <Link
                          href="/tambah-anak"
                  ...
                  children.map((child) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full border ${getStatusBadgeColor(
                            child.statusGizi
                          )}`}
                        >
                          {child.statusGizi}
